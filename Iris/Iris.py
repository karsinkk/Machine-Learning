...
X = np.matrix(X)
y = np.hstack((
     np.matrix(list(map(lambda x: int(x == "Iris-setosa"), Y))).T,
...
def cost(_x, _y, _theta, _lamb):
    j = -(1 / m) * (_y.T.dot(log10(sigmoid(_x.dot(_theta.T))).T) +
                    (
                    (1 - _y).T.dot(log10(sigmoid(1 - (_x.dot(_theta.T)))).T))
                    ) + (_lamb / 2 * m) * (_theta.dot(_theta.T))
...
def descent(_x, _y, _theta, _lamb, _alpha, _iter):
    ...
    for i in range(1, _iter + 1):
        g = _theta
        g[0, :] = g[0, :] - (_alpha / m) * (
            sigmoid(_x.dot(g)) - _y[:, 0]).T.dot(_x[:, 0])

        g[1:, ] = g[1:, ] * (1 - (_alpha * _lamb / m)) - (
            _alpha / m) * (_x[:, 1:]).T.dot(sigmoid(_x.dot(g)) - _y)
